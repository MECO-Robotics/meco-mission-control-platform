...
Fresh seed data includes eight fictional members with realistic names, with `example.com` emails, disciplines, and planned attendance. Fresh tutorial snapshots record one demo attendance hour for each on the current UTC date so they appear in Available Students. Attendance dates stay fixed during an active session; restart/reset refreshes them. They are part of the Tutorial Season and return on development startup or tutorial baseline reset. Development resets discard local edits; an existing production snapshot is not modified or backfilled. No schema migration is needed. Public demo responses use realistic fictional pseudonyms and omit contact details and private attendance plans. They include non-privileged student/mentor/external categories and a synthetic six-hour Tuesday/Thursday plan for each member so the directory and local availability views share a usable roster. All report participant, mentor, and requester references use those same anonymized roster IDs. Newly added demo members remain unassigned and available for local task assignment.

### Tutorial fixture dates

Fresh seed data and tutorial starts/baseline resets use the current UTC month and Monday–Sunday week. Seasons span the month; fixture activity is scaled chronologically into the part of the week inside that month, including month/year boundaries. Recorded work logs, QA, tests and attendance never occur after the snapshot clock; later planned dates can extend through Sunday. Task actual hours are derived from work logs on load and every mutation; task commands no longer accept authored actual-hour totals. Each start/reset reads the clock again; active sessions keep their dates and edits until reset. Session exit still restores the pre-tutorial workspace. Stored application data is not re-dated.
