...
repository's existing `prisma db push` deployment step. Keep legacy mobile JWT
issuance disabled after supported clients use the opaque refresh flow; if a
transition window is required, set both the enable flag and an explicit UTC
cutoff.
